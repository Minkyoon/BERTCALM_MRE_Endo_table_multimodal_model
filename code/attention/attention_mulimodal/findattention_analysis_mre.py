...
for fold in range(10):
    ...
    for i in tp:
        ...
        top_scores, top_indices = torch.topk(A_raw_mre, k=4, largest=True, dim=1)
        bottom_scores, bottom_indices = torch.topk(A_raw_mre, k=4, largest=False, dim=1)
        
        # Save images with highest and lowest attention scores
        data_csv = pd.read_csv(f'/home/jsy/2023_CD_MRE/data/label/{mre_serial_number}.csv')
        for scores, indices, folder in [(top_scores, top_indices, 'tp_up'), (bottom_scores, bottom_indices, 'tp_down')]:
            ...
            for score, index in zip(scores[0], indices[0]):
                ...
                output_dir = f'/home/minkyoon/2023_CLAM_MUTLIMODAL/code/attention/attention_mulimodal/image/{folder}_mre'
